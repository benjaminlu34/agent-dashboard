...
- `depends_on`: issue numbers that must reach `Needs Human Approval` (or `Done`) before this task can be promoted.
...
- `CHAINED`: can be promoted only if all `depends_on` issues have reached `Needs Human Approval` (or `Done`).
  Overlaps with `Ready`/`In Progress`/`In Review` issues remain blocked; overlaps with prerequisites already in
  `Needs Human Approval` are allowed so chained work does not wait on manual `Done`.
