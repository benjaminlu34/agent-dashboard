...
export async function runPreflightCheck({
  ...
  if (typeof role !== "string" || role.trim().length === 0) {
    return {
      statusCode: 400,
      payload: { error: "query parameter 'role' is required" },
    };
  }
  ...
  if (!UPPERCASE_ROLE_RE.test(normalizedRole)) {
    return {
      statusCode: 400,
      payload: { error: "role must be a valid filename token (letters, digits, underscore)" },
    };
  }
  ...
  try {
    ...
    if (error instanceof AgentContextBundleError && error?.details?.path === PROJECT_IDENTITY_PATH) {
      return {
        statusCode: 200,
        payload: buildProjectIdentityFailure({
          normalizedRole,
...
export function buildPreflightHandler({
  ...
  return async function preflightHandler(request, reply) {
    const { statusCode, payload } = await runPreflightCheck({
